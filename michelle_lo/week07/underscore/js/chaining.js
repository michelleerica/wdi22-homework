...
names = _.pluck(people, 'username')

_.each(names, function(name){
  var compiled = _.template("hello <%= name %>, you don't look a day over 20");
...
var detailedInfo = []
//
// _.each( startingData, function (details) {
// detailedInfo =_.unzip(details);
// detailedInfo = _.object(detailedInfo);
// detailedInfo = _.invert(detailedInfo);
// // var year = _.findWhere(detailedInfo, _.isNumber) ;
// // year-=100;
// detailedInfo.born -=100;
// detailedInfo.firstName = detailedInfo.firstName[0].toUpperCase() + detailedInfo.firstName.slice(1).toLowerCase();
// detailedInfo.lastName = detailedInfo.lastName[0].toUpperCase() + detailedInfo.lastName.slice(1).toLowerCase();




var  detailsI = _.map( startingData, function (details) {
detailedInfo =_.unzip(details);
detailedInfo = _.object(detailedInfo);
detailedInfo = _.invert(detailedInfo);
// var year = _.findWhere(detailedInfo, _.isNumber) ;
...
detailedInfo.firstName = detailedInfo.firstName[0].toUpperCase() + detailedInfo.firstName.slice(1).toLowerCase();
detailedInfo.lastName = detailedInfo.lastName[0].toUpperCase() + detailedInfo.lastName.slice(1).toLowerCase();
// detailedInfo.lastName =detailedInfo.lastName.toUpperCase();
...
// firstName = _.pluck(detailsI, 'firstName')
// lastName = _.pluck(detailsI, 'lastName')
// year = _.pluck(detailsI, 'born')

_.each(detailsI, function(person){
  var compiled = _.template("<%= firstname %> <%= lastname %>, was born in <%= born %>");
  console.log(compiled({firstname: person.firstName, lastname: person.lastName, born: person.born} ));
  // console.log(person.firstName, person.lastName, person.born, "this works");
})
